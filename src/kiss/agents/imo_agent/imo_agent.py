...
Does the solution meet the validation criteria above? Respond with exactly "PASS" or "FAIL" on the first line, followed by a brief explanation.
"""
...
class IMOAgent(Base):
    ...
    def _call_model(self, prompt: str, label: str) -> str:
        cfg = config_module.DEFAULT_CONFIG.imo.imo_agent
        agent = KISSAgent(f"{self.name}-{label}")
    ...
    def _solve(self, problem_statement: str) -> str:
        return self._call_model(SOLVER_SYSTEM + problem_statement, "Solver")

    def _self_improve(self, problem_statement: str, solution: str) -> str:
        prompt = (
            SOLVER_SYSTEM + problem_statement
            + "\n\n### Your Previous Solution ###\n\n" + solution
            + "\n\n### Improvement Instruction ###\n\n" + SELF_IMPROVEMENT_PROMPT
        )
        return self._call_model(prompt, "SelfImprove")

    def _verify(self, problem_statement: str, solution: str) -> tuple[str, bool]:
        detailed_solution = _extract_detailed_solution(solution)
        prompt = (
            VERIFIER_SYSTEM + problem_statement
            + VERIFIER_SOLUTION_HEADER + detailed_solution
            + VERIFIER_FOOTER
        )
        verification = self._call_model(prompt, "Verifier")
        check_prompt = (
        ...
        check = self._call_model(check_prompt, "CorrectnessCheck")
    ...
    def _correct(self, problem_statement: str, solution: str, bug_report: str) -> str:
        ...
        return self._call_model(prompt, "Corrector")

    def _single_run(self, problem_statement: str) -> str | None:
        cfg = config_module.DEFAULT_CONFIG.imo.imo_agent

        solution = self._solve(problem_statement)
        solution = self._self_improve(problem_statement, solution)
...
IMO_2025_PROBLEMS = [
    ...
    },
    {
        ...
        "avg_score": 3.306,
        "statement": (
            "Let $\\Omega$ and $\\Gamma$ be circles with centres $M$ and $N$, "
    ...
    },
    {
        ...
        "validation": (
            "The solution must characterize all valid a_1. The answer is all positive "
            "integers of the form 12^e * 6 * l where e, l >= 0 and gcd(l, 10) = 1. "
            "Equivalently, a_1 must be a multiple of 6 whose odd part is coprime to 5. "
            "The proof must show (1) all such a_1 produce infinite sequences, and "
            "(2) no other a_1 works, by proving every term must be divisible by 6 "
            "and analyzing the recurrence psi(x) = sum of three largest proper divisors."
        ),
    },
    {
        "number": 5,
        ...
        "statement": (
            ...
            "If a player cannot choose a suitable $x_n$, the game ends and the other "
            "player wins. If the game goes on forever, neither player wins. All chosen "
            "numbers are known to both players.\n\n"
    ...
    },
    {
        ...
        "statement": (
            ...
            "on the grid some rectangular tiles, possibly of different sizes, such that "
...
def validate_solution(
    ...
    cfg = config_module.DEFAULT_CONFIG.imo.imo_agent
    agent = KISSAgent("Validator")
    model_config: dict = {
        "temperature": cfg.temperature,
        "top_p": cfg.top_p,
    }
    prompt = (
        VALIDATION_SYSTEM + problem_statement
        + VERIFIER_SOLUTION_HEADER + solution
        + VALIDATION_CRITERIA_HEADER + validation
        + VALIDATION_FOOTER
    )
    safe_prompt = prompt.replace("{", "{{").replace("}", "}}")
    result = agent.run(
